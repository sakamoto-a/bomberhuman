...
function game_pad_update() {
  ...
  for(let i=0;i < num;i++){
    ...
    if (gamepad.mapping == "standard") {
      switch (i) {
        case 0:
          if (gamepad.button[12] || (gamepad.axes[1] < -0.5)) {
            game_data.buttons("up", 1);
          } else {
            game_data.buttons("up", 0);
          }
          if (gamepad.button[13] || (gamepad.axes[1] > 0.5)) {
            game_data.buttons("down", 1);
          } else {
            game_data.buttons("down", 0);
          }
          if (gamepad.button[14] || (gamepad.axes[0] < -0.5)) {
            game_data.buttons("left", 1);
          } else {
            game_data.buttons("left", 0);
          }
          if (gamepad.button[15] || (gamepad.axes[0] > 0.5)) {
